  ...
  stages { // Defines the different stages of the pipeline
    stage('Checkout Source') { // Defines the 'Checkout Source' stage
      steps { // Specifies the steps to be executed within this stage
        git 'https://github.com/Salah-Mensi/javulna.git' // Retrieves the source code from the specified GitHub repository
      }
    }
...
stage ('docker run container') { // Defines the 'docker run container' stage
    steps { // Specifies the steps to be executed within this stage
        ...
        sh 'docker run --name app -it -d -p 9002:8080 javulna-0.1' // Runs a new Docker container named 'app' based on the 'javulna-0.1' image, with port mapping from 8080 to 9000, in detached mode (-d), and allocates a pseudo-TTY (-it)
     }
   }
 }
}
